...
local command = function (name, repl, opts)
  ...
  if repl_type == 'function' then
    local key = trc.util.get_key_for_fn(repl, trc.g.command_callbacks)
    trc.g.command_callbacks[key] = repl
    repl = 'lua trc.g.command_callbacks.' .. key .. '()'
