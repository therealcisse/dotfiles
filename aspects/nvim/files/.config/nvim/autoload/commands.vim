...
function! s:open_on_github(file, range) abort
  let l:git_dir=git#get_git_dir(a:file)
  if l:git_dir == ''
    call functions#echoerr('No .git directory found above ' . a:file)
    return
  endif
...
function! commands#find(args) abort
    set errorformat+=%f
...
function! commands#marked(...) abort
  if a:0 == 0
...
function! commands#open_on_github(...) abort range
  let l:range=''
  if a:0 == 0
    let l:files=[expand('%')]
...
function! commands#preview(...) abort
  if executable('open')
    call call('commands#marked', a:000)
  elseif executable('glow')
    call call('commands#glow', a:000)
  else
    echoerr 'No "open" or "glow" executable found'
  endif
endfunction

function! commands#typecheck() abort
  " Make subsequent `:make` work (eg. invoked by Dispatch's `m<CR>` mapping).
  compiler tsc
